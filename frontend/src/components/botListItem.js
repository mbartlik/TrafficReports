...
const BotListItem = ({ bot, linkPath, showButtons = false, onDetailsClick, isMobile }) => {
  const chatButton = (
    <Link
      to={linkPath || `/bot/${bot.id}`}
      style={{
        ...styles.actionButton,
        ...styles.chatButton,
        ...(isMobile ? { ...styles.mobileButton, marginLeft: 'auto', padding: 0 } : {}),
      }}
    >
      Chat
    </Link>
  ...
  return (
    <li
      style={{ marginBottom: '1rem', ...(isMobile ? styles.mobileList : {}) }}
      className="bot-list-item"
    >
      <div style={{ display: 'flex', alignItems: 'center' }}>
        <h3 style={{ marginRight: '1rem', ...(isMobile ? styles.mobileSubHeader : {}) }}>
          {bot.name}
        </h3>
